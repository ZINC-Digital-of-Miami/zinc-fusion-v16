...
- Each opportunity row links to its highest verified `vegas.event_impact` event when such rows exist for the account; rows without verified impact linkage fall back to the nearest upcoming verified event (`events[0]` by soonest date) for demand-window context (`eventDaysUntil`). No synthetic per-account event-impact matrix is ever generated.
- Modeled signals (`opportunityScore`, `eventPressure`, `expectedSpend`, `hospitalityImpact`, `phqMultiplier`, `zfusionScore`) are computed only from verified `vegas.customer_scores` / `vegas.event_impact` rows and verified event attendance; they are explicit `null` when those rows are absent. `expectedSpend` resolves only from `vegas.event_impact.metadata` spend fields — restaurant/Glide metadata is never used as a spend source. The trusted fill pipeline never fabricates them.
- Opportunity ranking uses real signals first: customers before prospects, then `estimatedOilLbsPerWeek` desc, then Glide field completeness (service cadence, oil type, contact, capacity, event link) ahead of any modeled score, then event adjacency (`eventDaysUntil`), then verified `zfusionScore`/`opportunityScore` as final tie-breakers.
- `customerStatus` is `customer` for every Glide-synced restaurant row (`metadata.source = glide`) — Glide is the production CRM of existing serviced accounts, so sparse cadence fields surface as missing telemetry, not as invented prospects. Non-Glide rows (once a verified non-customer universe lands) classify as `customer` only with a verified service schedule, otherwise `prospect`.
...
- `/api/vegas/intel` merges the AI snapshot from `app/config/vegas-intel-ai.json` (`readAiSnapshot`), returning `cards` (Upcoming Events, AI Sales Strategy, Restaurant Accounts, Fryer Tracking) plus an `ai` envelope (`enabled`, `source`, `model`, `generatedAt`, `refreshScheduleEt`). Card bodies are always the deterministic server-computed text recomputed from verified event counts, Glide counts, and estimated weekly oil aggregates (no request-time external market pulls); snapshot title/instruction/provenance fields win when present.
- The Vegas body page renders the V15-parity sales workspace: Glide table coverage cards for all 8 source groups, the four segment cards (`all`/`customers`/`prospects`/`events`), shift service coverage, the lead view with fail-closed net-new copy, event windows with countdown urgency colors, and ranked opportunity cards with explicit missing-field chips. This surface is pinned by `scripts/tests/test_vegas_intel_completeness_sort_contract.py`, which runs as a blocking CI gate.
