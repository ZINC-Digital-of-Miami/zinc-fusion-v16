...
- Modeled signals (`opportunityScore`, `eventPressure`, `expectedSpend`, `hospitalityImpact`, `phqMultiplier`, `zfusionScore`) are computed only from verified `vegas.customer_scores` / `vegas.event_impact` rows and verified event attendance; they are explicit `null` when those rows are absent. The trusted fill pipeline never fabricates them.
