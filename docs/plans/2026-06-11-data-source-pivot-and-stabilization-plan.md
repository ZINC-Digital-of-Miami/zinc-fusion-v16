...
- **CP-3** Chart freshness: EOD-only vs +Yahoo-1h; AGENTS rule 19 amendment
- **CP-4** Glide sync scheduler: pg_cron+http port vs launchd lane
- **CP-5** SQL-cron source swap detail (now concrete): financialdata.net `commodity-prices` ZL/ZS/ZM/CL/HO + `index-prices` ^VIX/^OVX + forex USDCNH (or Yahoo CNY=X); FRED USEPUINDXD; CFTC Socrata COT. `economic-calendar`/`fed-press-releases` returned HTTP 400 without params — parameterization TBD if ever needed (narrative-only).
